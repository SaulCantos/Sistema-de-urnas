...
public class BocaDeUrnasApp {
    ...
    private static JTextField txtNombreEstudiante;
    private static JTextField txtApellidoEstudiante;
    private static JComboBox<String> comboParalelo;
    
    private static List<Candidato.Estudiante> estudiantesRegistrados = new ArrayList<>();
    ...
    private static JPanel crearPanelAdministrativa() {
        ...
        gbc.gridx = 0;
        gbc.gridy = 2;
        panelFormulario.add(new JLabel("Seleccione Paralelo:"), gbc);

        JComboBox<String> comboParalelo = new JComboBox<>(paralelos.toArray(new String[0]));
        gbc.gridx = 1;
        panelFormulario.add(comboParalelo, gbc);

        gbc.gridx = 0;
        gbc.gridy = 3;
        panelFormulario.add(new JLabel("Nombre Estudiante:"), gbc);

        gbc.gridx = 1;
        panelFormulario.add(txtNombreEstudiante = new JTextField(15), gbc);

        gbc.gridx = 0;
        gbc.gridy = 4;
        panelFormulario.add(new JLabel("Apellido Estudiante:"), gbc);

        gbc.gridx = 1;
        panelFormulario.add(txtApellidoEstudiante = new JTextField(15), gbc);
        ...
        JPanel panelMesasCursos = crearPanelMesasCursos();

        tabbedPaneAdmin.addTab("Registrar Mesas y Cursos", null, panelFormulario, "Registrar Mesas y Cursos");
        tabbedPaneAdmin.addTab("Reporte", null, panelMesasCursos, "Reporte de Mesas y Cursos");

        panelAdministrativa.add(tabbedPaneAdmin, BorderLayout.CENTER);
    ...
    private static JPanel crearPanelMesasCursos() {
        JPanel panelMesasCursos = new JPanel(new BorderLayout());

        JLabel labelTitulo = new JLabel("Reporte de Mesas y Cursos");
        labelTitulo.setHorizontalAlignment(JLabel.CENTER);
        panelMesasCursos.add(labelTitulo, BorderLayout.NORTH);

        JTextArea areaReporte = new JTextArea();
        areaReporte.setEditable(false);

        areaReporte.append("Mesas:\n");
        for (String mesa : cursosAdministrativos) {
            areaReporte.append(mesa);
        }

        areaReporte.append("\nCursos:\n");
        for (String curso : cursosAcademicos) {
            areaReporte.append(curso);
        }

        JScrollPane scrollPane = new JScrollPane(areaReporte);
        panelMesasCursos.add(scrollPane, BorderLayout.CENTER);

        return panelMesasCursos;
    }
    
    private static void registrarMesaYCurso1() {
        String mesaSeleccionada = (String) comboCursosAdministrativos.getSelectedItem();
        String cursoSeleccionado = (String) comboCursosAcademicos.getSelectedItem();
        String paraleloSeleccionado = (String) comboParalelo.getSelectedItem();
        String nombreEstudiante = txtNombreEstudiante.getText();
        String apellidoEstudiante = txtApellidoEstudiante.getText();

        if (mesaSeleccionada != null && cursoSeleccionado != null && paraleloSeleccionado != null
                && nombreEstudiante != null && apellidoEstudiante != null) {

            if (estudiantesRegistrados == null) {
                estudiantesRegistrados = new ArrayList<>();
            }

            Candidato.Estudiante nuevoEstudiante = Candidato.Estudiante(nombreEstudiante, apellidoEstudiante, paraleloSeleccionado);
            estudiantesRegistrados.add(nuevoEstudiante);

            JOptionPane.showMessageDialog(null, "Se ha registrado la mesa '" + mesaSeleccionada + "' para el curso de '" +
                    cursoSeleccionado + "' en el paralelo '" + paraleloSeleccionado + "' con el estudiante: " +
    ...
    private static void actualizarReporteEstudiantes() {
        StringBuilder reporte = new StringBuilder("Estudiantes Registrados:\n");
        for (Candidato.Estudiante estudiante : estudiantesRegistrados) {
            reporte.append("Nombre: ").append(estudiante.getNombre()).append(" ")
                    .append(estudiante.getApellido()).append(", Paralelo: ")
                    .append(estudiante.getParalelo()).append("\n");
        }

        JTextArea areaReporte = new JTextArea(reporte.toString());
        areaReporte.setEditable(false);

        JPanel panelMesasCursos = (JPanel) ((JScrollPane) ((JTabbedPane) SwingUtilities.getAncestorOfClass(JTabbedPane.class, panelEstadisticas))
                .getComponentAt(1)).getViewport().getView();
        panelMesasCursos.removeAll();
        panelMesasCursos.add(areaReporte, BorderLayout.CENTER);

        panelMesasCursos.revalidate();
        panelMesasCursos.repaint();
    }
...
class Candidato {
    ...
    public static Candidato.Estudiante Estudiante(String nombreEstudiante, String apellidoEstudiante,
    ...
    static class Estudiante {
        ...
        public Estudiante(String nombre, String apellido, String paralelo) {
            this.nombre = nombre;
            this.apellido = apellido;
    ...
    static class EstudianteCandidato {
        private int id;
        private String nombre;
        private String apellido;
        private String partido;
        ...
        public void Candidato(int id, String nombre, String apellido, String partido, int numeroLista) {
            this.id = id;
            this.nombre = nombre;
            this.apellido = apellido;
            this.partido = partido;
        ...
        public void agregarEstudiante(String nombreEstudiante, String apellidoEstudiante, String paraleloSeleccionado) {
            Estudiante nuevoEstudiante = new Estudiante(nombreEstudiante, apellidoEstudiante, paraleloSeleccionado);
        }
